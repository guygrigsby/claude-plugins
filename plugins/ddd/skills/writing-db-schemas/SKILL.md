...
2. **Every reference is a foreign key, and every FK declares ON DELETE.**
...
3. **A fact that happens later is its own table.** `returned_at`,
...
7. **NOT NULL is the default.** NULL means "a value exists but we don't know
   it yet" — never "hasn't happened" (rule 3) and never "not applicable"
   (rule 4). Every surviving nullable column states what NULL means in a
   comment beside it, or loses its nullability.
8. **5NF, no derivable columns.** A value identical across an operation's
   child rows (date, memo, author, client) belongs on the parent, once.
   Display names come from JOINs, not snapshot copies. A deliberate exception
   is recorded in a comment beside the column — an unrecorded exception is a
   loophole.
9. **No compound-key ceremony.** `UNIQUE (scope_id, id)` beside a
...
10. **Every index names the query it serves**, in a comment. A rule SQL
