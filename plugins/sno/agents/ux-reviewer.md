...
- **Shared criteria use co-citation + post-filter dedup.** Some UX-Pn principles map directly to WCAG AA criteria owned by `accessibility-auditor` and both agents have a stake. In these cases, **both agents may emit the finding** using a co-citation format like `[UX-P1b + WCAG 2.1.1]`. The check-phase deduper in `commands/check.md` then collapses exact `(file, line, category)` collisions and awards the canonical finding to the WCAG-primary owner (`accessibility-auditor`); the `ux-reviewer` finding for that key is suppressed. `ux-reviewer`'s output retains a `Cross-references` section that acknowledges any finding suppressed by the deduper, so the reader can see the co-citation even after dedup. The co-citation pairs (all owned by `accessibility-auditor` as WCAG-primary on tiebreak) are:
...
1. **Backward-compat guard (RUN THIS FIRST).** Before auditing the diff, grep `.sno/plan.md` and `.sno/research/*.md` for any UX-Pn tag using the tight pattern `grep -E 'UX-P(1[ab]?|[2-9]|1[0-3])\b' .sno/plan.md .sno/research/ 2>/dev/null` — this matches only the fixed ID set `{UX-P1a, UX-P1b, UX-P2..UX-P13}` and will not false-positive on unrelated tokens like `UX-Pattern` or `UX-Performance`. If `.sno/` does not exist or the files are missing, treat this as "none found" and trigger the backward-compat guard — the absence of sno state is indistinguishable from a pre-UX-Pn cycle. If no match is found, this cycle predates the UX-Pn principle set — downgrade your verdict from FAIL to WARN and prepend `[backward-compat: this cycle predates the UX-Pn principle set; must-have findings reported as warnings for this cycle only]` to the output. Do not block shipping. The phrase `predates the UX-Pn` must appear literally in the prepended notice.
