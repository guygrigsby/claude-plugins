...
- **[db-schemas](plugins/db-schemas/CLAUDE.md)** -- integrity-first SQL DDL whenever schemas or migrations are written (natural keys preferred to surrogates, ON DELETE on every FK, later facts as their own tables, enum tables for closed vocabularies, NOT NULL by default, indexes naming their queries)
